...
- **Docker/cluster actions need a human in the loop, even for the orchestrator.** Spawning or tearing down a cluster (`POST /topics/<id>/spawn|teardown`), killing a specific container (`docker stop`/`docker kill`), and editing `.claude/settings*.json` to self-grant permissions are all blocked by the Claude Code auto-mode permission classifier — for background subagents *and* for the orchestrator's own direct tool calls, by design (self-escalating permissions stays blocked even after the human adds a narrowly-scoped Bash rule covering the surrounding `curl` calls). When a live/acceptance pass needs a real spawn or a broker/worker kill, ask the human to either run that one command themselves or add the permission rule themselves — don't try to route around a denial via a different tool or file edit. Read-only observation (`curl` GETs, `docker ps`, screenshots) is unaffected. Before touching Docker at all, identify which worktree owns any already-running `sparkpb` cluster via `docker inspect <container> --format '{{index .Config.Labels "com.docker.compose.project.working_dir"}}'` — the single-slot design (#38) means a live cluster may belong to a concurrent session's worktree, not yours; never spawn/kill/teardown against a cluster you haven't confirmed you own. Established during issue #59's live acceptance pass (2026-07-20/21).
