...
## Focus areas
- **CI pipelines** — build, test, lint stages; keep them fast and fail-fast. Where a CI provider is being set up or changed, consider enforcing the branch-naming and commit-message conventions in `.claude/GITHUB_CONVENTIONS.md` (e.g. a commitlint stage) rather than leaving them as unenforced documentation. `.github/workflows/ci.yml` (pull_request → pytest) is what actually enforces `GITHUB_CONVENTIONS.md`'s "all CI checks must pass before merge" rule — keep it working, since parallel worktrees merging concurrently depend on it to catch a bad merge/rebase before `main` (which auto-deploys on push via `deploy-lan.yml`).
