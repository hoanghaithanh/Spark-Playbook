...
## When invoked
1. Check locally first: Grep/Glob the codebase and package manifests for an equivalent already in use or already a dependency. If one exists, say so — that's usually the right answer before anything external.
