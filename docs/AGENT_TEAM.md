...
A set of Claude Code subagents, one per SDLC role, used to drive this repo's own development
process. The set was originally built as a template meant to be dropped into any project's
`.claude/agents/` directory — that's still true, and the "Install" section below is written with
that portability in mind — but in *this* repo it's already installed and wired up: `CLAUDE.md` is
what turns the agents below into the orchestration process this project actually follows.
...
This repo already has `.claude/agents/` in place, so nothing further is needed to use the team here. To reuse the same agent set in another project:
...
The pipeline order and the definition-of-done live in **`CLAUDE.md`**, which the main Claude Code session reads automatically. That's what turns the individual agents into a process — see `CLAUDE.md` for how this specific repo runs its sprints and pipeline.
