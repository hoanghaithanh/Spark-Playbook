...
| 40 | Multi-broker Kafka cluster & monitor — 5 sub-stories: (a) user-configurable "Kafka" section in the cluster-config drawer (1-5 brokers, default 3, RF=3/min-isr=2, folded into the existing single Spawn/Teardown action), (b) Kafka observability data layer (CLI-shellout, not KafkaAdminClient — see #50's ADR dead end), (c) JMX exporter for heap/GC and produce/consume latency plus per-broker idle-ratio metrics, (d) 4th tab in the existing Cluster Monitor panel built against a specific design mockup, (e) broker-kill fault-tolerance demo (ISR shrink/leader re-election) | L | [docs/requirements/multi-broker-kafka-cluster.md](requirements/multi-broker-kafka-cluster.md) | Backlog — requirements formalized 2026-07-19 (requirements-analyst) per the human-approved plan `for-18-i-want-lazy-candle.md`; explicitly amends `docs/architecture/kafka-streaming-infra.md` Decision D1 (reverses "Kafka is not a user-facing toggle" — see the new doc's Amends section). All 5 sub-stories (US-MBK1-5) have given/then acceptance criteria; two items flagged as genuinely open (exact JMX MBean names, exact Kafka CLI output-parsing shapes) rather than guessed. Filed under release milestone `v1.2 — Multi-Broker Kafka Cluster & Monitor` (GitHub milestone [#15](https://github.com/hoanghaithanh/Spark-Playbook/milestone/15)), not yet pulled into a sprint — GitHub issues for the 5 sub-stories are now filed: (a) [#56](https://github.com/hoanghaithanh/Spark-Playbook/issues/56), (b) [#57](https://github.com/hoanghaithanh/Spark-Playbook/issues/57), (c) [#58](https://github.com/hoanghaithanh/Spark-Playbook/issues/58), (d) [#59](https://github.com/hoanghaithanh/Spark-Playbook/issues/59), (e) [#60](https://github.com/hoanghaithanh/Spark-Playbook/issues/60). **Sequencing note:** must land before v1.1's still-unstarted sub-stories (#52-#55, row #18) — those build the streaming producer/job/dashboard against whatever broker topology is running, so they should target the multi-broker cluster rather than inherit the single-node one from #50 (row #19). Mirrors how row #19 was itself a prerequisite consumed by row #18. **Sub-story status:** (a)/#56 — Done, human sign-off 2026-07-20 (`docs/qa/multi-broker-kafka-cluster-acceptance.md`); (b)/#57 — Done, human sign-off 2026-07-20 (`docs/qa/kafka-observability-layer-acceptance.md`); (c)/#58 — Done, human sign-off 2026-07-20 (`docs/qa/jmx-exporter-acceptance.md`), built in Sprint 12 (GitHub milestone [#16](https://github.com/hoanghaithanh/Spark-Playbook/milestone/16), 2026-07-20 - 2026-07-24) in a dedicated worktree the same day it was pulled in, alongside that sprint's 4 Kafka-curriculum issues (#62-#65, no file overlap with #62 confirmed). Bakes the `jmx_prometheus_javaagent` into the Kafka broker image at build time (`compose/Dockerfile.kafka`), attached via `-javaagent`/`KAFKA_OPTS`, loopback-bound, no new host-published port (7071 confirmed absent from `docker port`/`docker ps` on all 3 brokers). Code-reviewer found 1 Major (the ADR's literal `KAFKA_OPTS` draft as a plain `environment:` entry would leak into every `docker exec ... kafka-*.sh` CLI shellout from sub-story (b)'s observability layer, each spawning its own JVM trying to rebind the loopback agent port and crashing with `BindException` — silently breaking US-MBK2's entire CLI layer) plus 2 Minor/1 Nit findings; the Major was fixed by scoping `KAFKA_OPTS` into the `command:` override instead of `environment:` (documented in the template's own DEVIATION comment), fixed and re-verified; the 2 Minors/1 Nit accepted as-is. All 5 US-MBK3 given/thens validated live against a real 3-broker Docker cluster with real produce/fetch traffic (500 messages, `acks=all`): the `command:`-vs-`environment:` fix holds end-to-end (CLI admin-plane and JMX scrape coexist via a shared `asyncio.gather`, no `BindException`), and real JVM-level heap%/p99-latency/idle-ratio numbers cross-checked exactly against independently hand-run `docker exec ... wget .../metrics` output. 443 unit tests passing (`tests/unit/test_kafka_stats_jmx.py` new, `test_collector_kafka.py` extended), no regressions. **Merged 2026-07-20** — landed on `main` via PR [#67](https://github.com/hoanghaithanh/Spark-Playbook/pull/67) (merge commit `3f13b43`, containing `9c6c073 feat(kafka): JMX exporter for broker heap/latency/idle metrics (#58)`); the local worktree/branch (`.claude/worktrees/issue-58-jmx-exporter`, `worktree-issue-58-jmx-exporter`) has been removed. GitHub issue [#58](https://github.com/hoanghaithanh/Spark-Playbook/issues/58) is **closed** (verified via `gh issue view 58`). (d)/#59 — Done, human sign-off 2026-07-20 (`docs/qa/kafka-cluster-monitor-panel-ui-acceptance.md`), built as a 4th tab in the existing Cluster Monitor panel fed by the same shared collector/single SSE connection (not a new panel, not manifest-gated); applied 3 documented deviations from the DesignSync mockup (no `Suggestion:` line — no suggestion field on the Kafka models, no historical time-range tabs — no data store backs them, no broker-detail drill-down — no history buffers on `KafkaBrokerStat`, though consumer-group per-partition lag drill-down is implemented via native `<details>` since that data does exist). 10 new unit tests (`tests/unit/test_dashboard_kafka.py`), 459 total passing, no regressions. Code-reviewed — no Blockers, 2 cosmetic Nits accepted as-is. Live-acceptance-validated across 3 rounds against a real 3-broker Kafka spawn, including a real broker-kill test confirming offline marking and controller re-election, independently cross-checked against real `kafka-metadata-quorum.sh` CLI output. **Merged 2026-07-20** — landed on `main` via PR [#69](https://github.com/hoanghaithanh/Spark-Playbook/pull/69) (merge commit `829b4ff`, containing `9f449ef feat(kafka): Kafka Cluster Monitor panel UI (#59)`). GitHub issue [#59](https://github.com/hoanghaithanh/Spark-Playbook/issues/59) is **closed** (verified via `gh issue view 59`, auto-closed by the `Fixes #59` commit keyword). ISR-shrink-feed population deliberately stays out of scope here, blocked on (e)/#60, which owns that logic. Only (e)/#60 remains open, unstarted, under v1.2 (milestone #15) — 4 of 5 sub-stories (a/b/c/d) now done; milestone #15 currently tracks 3 closed / 1 open (#56, #57, #59 closed, #60 open — (c)/#58 lives under Sprint 12/milestone #16 instead, per its own row text above). |
| 41 | Kafka curriculum topic: `kafka-architecture-kraft` — brokers/controllers/partitions/replication in a KRaft (no ZooKeeper) cluster, contrasted against the legacy ZooKeeper-coordinated architecture | M | [docs/requirements/kafka-curriculum.md](requirements/kafka-curriculum.md) (US-KC1) | Done (Sprint 12) — split out of the former combined row #41 2026-07-20 (project-manager), matching the "one topic, one story" grain the rest of this table already uses for curriculum content. Size bumped S->M 2026-07-20 to absorb the one-time topics-index grouping build (D-KC1: new `track` manifest field, `app/topics/loader.py` change, template change) bundled into this issue since it is the first Kafka topic to ship and needs somewhere to render. Pulled into **Sprint 12 (GitHub milestone [#16](https://github.com/hoanghaithanh/Spark-Playbook/milestone/16), 2026-07-20 – 2026-07-24)** 2026-07-20. Implemented, tested, code-reviewed (1 finding fixed — guard `track=None` from a malformed manifest), and live-acceptance-validated against a real 3-broker KRaft cluster: all 3 US-KC1 acceptance criteria plus the D-KC1 grouping criterion PASS (`docs/qa/kafka-architecture-kraft-acceptance.md`). Human has given final sign-off (2026-07-20). GitHub issue [#62](https://github.com/hoanghaithanh/Spark-Playbook/issues/62) closed 2026-07-20 — closed directly, no `Fixes #62` commit-message keyword at the time (same departure pattern as #56/#57/#50, not a new issue). **Merged 2026-07-20** — landed on `main` via PR [#66](https://github.com/hoanghaithanh/Spark-Playbook/pull/66) (merge commit `1e998f7`, "Kafka curriculum: kafka-architecture-kraft + topics-index grouping (#62)"); issue #62 confirmed still closed post-merge (`gh issue view 62`). Remaining Sprint 12 issues #63, #64, #65 still open (milestone #16: 2 closed / 3 open as of 2026-07-20). |
| 42 | Kafka curriculum topic: `kafka-topics-partitions` — partition count/key choice determining message ordering and distribution, keyed vs. unkeyed produce, per-partition ordering (no cross-partition guarantee) | S | [docs/requirements/kafka-curriculum.md](requirements/kafka-curriculum.md) (US-KC2) | Done (Sprint 12) — split out of the former combined row #41 2026-07-20. Fully buildable today, no blockers. Pulled into **Sprint 12 (GitHub milestone [#16](https://github.com/hoanghaithanh/Spark-Playbook/milestone/16), 2026-07-20 – 2026-07-24)** 2026-07-20. Implemented (`content/kafka-topics-partitions/`, `track: kafka`, `order: 2`, `requires_kafka: true`), tested (453 passed), code-reviewed (no Blockers — fixed a Minor: `KafkaProducer` now explicitly closed in the notebook), and live-acceptance-validated twice — once by the developer, independently re-run by test-engineer — against a real 3-broker Kafka cluster and real JupyterLab kernel, both with clean teardown confirmed: all 3 US-KC2 criteria PASS (`docs/qa/kafka-topics-partitions-acceptance.md`). **Merged 2026-07-20** — landed on `main` via PR [#68](https://github.com/hoanghaithanh/Spark-Playbook/pull/68) (merge commit `52ce58d`, containing `29b90c4 feat(content): add kafka-topics-partitions topic (US-KC2, closes #63)` plus a follow-up fix `520a7cf fix(content): close KafkaProducer in kafka-topics-partitions notebook`). GitHub issue [#63](https://github.com/hoanghaithanh/Spark-Playbook/issues/63) is **closed** (verified via `gh issue view 63`, auto-closed by the `Fixes #63` commit keyword). Sprint 12 (milestone #16) now 3 closed / 2 open (#64, #65 remain). |
| 43 | Kafka curriculum topic: `kafka-producers-delivery` — `acks`/idempotence/retries under induced failure, at-least-once vs. at-most-once vs. (practically) exactly-once producer-side behavior, measured not asserted | M | [docs/requirements/kafka-curriculum.md](requirements/kafka-curriculum.md) (US-KC3) | Done (Sprint 12) — split out of the former combined row #41 2026-07-20. **Status corrected 2026-07-20**: previously marked "fully buildable today, no blockers" — wrong. The architect's US-KC11 spike (`docs/architecture/kafka-curriculum.md` D-KC3) found `kafka-python==2.0.2` has no idempotent-producer support at all (confirmed directly against the library source). Still fully buildable today with no new dependency — the idempotence bullet uses the same CLI-subprocess fallback (`kafka-console-producer.sh --producer-property enable.idempotence=true`) already established for US-KC11; the `acks`-only bullets are unaffected (`acks` is a real `kafka-python` config). Pulled into **Sprint 12 (GitHub milestone [#16](https://github.com/hoanghaithanh/Spark-Playbook/milestone/16), 2026-07-20 – 2026-07-24)** 2026-07-20. Implemented (`content/kafka-producers-delivery/`, `track: kafka`, `order: 3`, `requires_kafka: true`), tested (`tests/unit/test_topics_loader.py::TestLoadRealKafkaProducersDeliveryTopic`, 467 passed, no regressions), code-reviewed (ship as-is, no unresolved findings), and live-acceptance-validated against a real 3-broker cluster with two genuine induced leader-broker restarts (`docs/qa/kafka-producers-delivery-acceptance.md`): all 5 US-KC3 acceptance criteria PASS — `acks=0` silently lost 2/60 messages with zero local exceptions raised, `acks=all`+idempotence (CLI) landed all 60 exactly once through a live retry, `acks=1`'s failure window is explained in `concept.md`, the summary table reflects real measured counts, and `concept.md` explains the deliberate `kafka-python`/CLI mixing. One non-blocking follow-up bug found during validation — the idempotence step's printed `docker exec` target is hardcoded to `spark-kafka-1` instead of resolving the actual partition leader, self-defeating roughly 1 run in 3 — filed as GitHub issue [#71](https://github.com/hoanghaithanh/Spark-Playbook/issues/71), not blocking this acceptance pass. **Merged 2026-07-21** — landed on `main` via PR [#72](https://github.com/hoanghaithanh/Spark-Playbook/pull/72) (merge commit `44543e5`). GitHub issue [#64](https://github.com/hoanghaithanh/Spark-Playbook/issues/64) is **closed** (auto-closed on merge). Sprint 12 (milestone #16) now 3 closed / 1 open (#65 remains, active in-progress work in a separate worktree/session). |
| 44 | Kafka curriculum topic: `kafka-consumers-groups` — offset commits, consumer-group rebalancing, partition-bounded parallelism ceiling, manual-vs-auto-commit crash/restart behavior | M | [docs/requirements/kafka-curriculum.md](requirements/kafka-curriculum.md) (US-KC4) | Done (Sprint 12) — split out of the former combined row #41 2026-07-20. Pulled into **Sprint 12 (GitHub milestone [#16](https://github.com/hoanghaithanh/Spark-Playbook/milestone/16), 2026-07-20 – 2026-07-24)** 2026-07-20; GitHub issue [#65](https://github.com/hoanghaithanh/Spark-Playbook/issues/65) filed and milestoned. Implemented: `content/kafka-consumers-groups/{manifest.yaml,concept.md,notebook.ipynb}` (4th Kafka-track topic), plus `tools/kafka_consumer_group/{member.py,README.md}` and `driver/playbook/consumer_group.py`. Unit-tested — 504 passing (grew from the 496 baseline as test-engineer/code-reviewer/developer rounds each added coverage). Code-reviewed — 2 Major findings (auto-commit-tick silent-data-loss risk, unguarded stdout-pipe deadlock risk) fixed pre-live-testing; no Blockers. **Live-acceptance-validated across 4 rounds** against a real 3-broker Kafka cluster (`docs/qa/kafka-consumers-groups-acceptance.md`), finding and fixing 4 real bugs along the way — each only reachable once the prior one was fixed, not noise: round 1 found [#73](https://github.com/hoanghaithanh/Spark-Playbook/issues/73) (`auto_offset_reset` defaulting to `"latest"`, hiding every fresh consumer group's backlog); round 2 found [#74](https://github.com/hoanghaithanh/Spark-Playbook/issues/74) (Section 5's restart cell could hang indefinitely on an undeadlined `close()`; Section 6 as timed caused deterministic 100% backlog loss instead of the intended partial-loss demo) and [#75](https://github.com/hoanghaithanh/Spark-Playbook/issues/75) (`--self-check` hardcoded partition 0, but the fixed test key hashes to partition 1 in this environment); round 3 found [#76](https://github.com/hoanghaithanh/Spark-Playbook/issues/76) (manual-commit's bare `consumer.commit()` committed kafka-python's ambient fetch position rather than the offset actually finished processing — the same "position races ahead of processing" defect already known for auto-commit, but on the manual path); round 4 confirmed all 4 US-KC4 acceptance criteria PASS, with 3 independent fresh runs of the manual-commit crash/restart all clean (broker-committed offset = 3 after processing 3, restart resumes at exactly offset 3, no message lost/duplicated, no hangs). Also filed, non-blocking and separate from this feature's correctness per the human's own direction: [#77](https://github.com/hoanghaithanh/Spark-Playbook/issues/77), a recurring Docker cluster-teardown anomaly hitting this session 4 times, root cause still unknown. No security-auditor pass — not triggered (no auth/secrets/PII/payments, consistent with KC1-KC3's own disposition). **Human has given final sign-off (2026-07-21).** GitHub issue [#65](https://github.com/hoanghaithanh/Spark-Playbook/issues/65) (and #73/#74/#75/#76) close on merge of this row's PR, per this repo's established close-at-merge convention. Sprint 12 (milestone #16) currently 4 closed / 3 open (#58, #62, #63, #64 closed; #65, #73, #76 open) — #65, #73, #76 all close on merge of this row's PR, which would leave milestone #16 fully closed. Bugs #74/#75/#77 are open but not milestoned into Sprint 12. Fourth and final of Sprint 12's 4 Kafka-curriculum stories to reach human sign-off. |
...
[#16](https://github.com/hoanghaithanh/Spark-Playbook/milestone/16), 2026-07-20 – 2026-07-24)**
...
## Sprint 12 date correction (2026-07-20, project-manager)

Sprint 12 was originally scoped/milestoned with a start date two weeks out
(2026-08-03 – 2026-08-07) despite work (issues #62-#65, plus #58 pulled in
same day per row #40) starting immediately. Flagged in the prior report;
human confirmed the fix. Recent sprint milestones (Sprint 10, 11) settled
into a Mon-Fri, 5-business-day cadence — Sprint 12 now follows that same
pattern, moved to **2026-07-20 – 2026-07-24** (today is Monday 2026-07-20).
GitHub milestone #16 updated via `gh api` PATCH: title renamed to
`Sprint 12 (2026-07-20 – 2026-07-24)`, `due_on` set to
`2026-07-24T00:00:00Z`. All literal date-range mentions above (rows #40,
#41-#44 and the "Sprint 12 execution" section) updated to match.
